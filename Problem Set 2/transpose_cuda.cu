...
void naiveTransposeKernel(const float *input, float *output, int n) {
  ...
  for (; j < end_j; j++) {
    // Output is a non-coalesced memory access (write). It touches 4
    // cache lines. No bank conflicts (no shared memory).
...
void shmemTransposeKernel(const float *input, float *output, int n) {
  ...
  __shared__ float data[64][65]; // Pad the shared memory by one
  ...
  k = 4 * threadIdx.y + 64 * blockIdx.x;
  end_k = k + 4;
  
  for(; k < end_k; k++) {
    output[i + n * k] = data[4 * threadIdx.y + (4 - (end_k - k))][threadIdx.x];
  }
}
